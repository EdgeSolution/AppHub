...
|Custom alarm notification|√	|It can be extended through a new plug-in mechanism. With this functions, user can monitor any status only with simple configuration, no coding|
|Alarm and device linkage|√| This funciotn can trige a default handler automatically for an alarm event|
